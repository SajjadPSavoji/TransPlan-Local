...
def im_clicks(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONUP:
        ...
        cv2.putText(image, str(im_ptnum), (x,y), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 1, cv2.LINE_AA) 
...
def wd_clicks(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONUP:
        ...
        wd_ptnum+=1
        cv2.circle(world,(x,y), 2, (255,255,255), 3)
        cv2.putText(world, str(wd_ptnum), (x,y), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2, cv2.LINE_AA) 
...
def main():
    ...
    np.save('homography_'+str(len(world_pts_collected))+'.npy',cv2hom)
    ...
    np.savetxt('homography_matrix_'+str(len(world_pts_collected))+'pts.txt',cv2hom[0])
    cv2.waitKey(0)
